...
const apiMocks = {
  getUser: (status?: number, body?: object) =>
    mockOctokit(
      ...
      "/user",
      status ?? 200,
      body ?? { login: "step-security" },
    ),
  getPull: (status?: number, body?: object) =>
    mockOctokit(
      "get",
      "/repos/step-security/test/pulls/101",
      status ?? 200,
  ...
  getReviews: (status?: number, body?: any) =>
    mockOctokit(
      "get",
      "/repos/step-security/test/pulls/101/reviews",
      status ?? 200,
      body ?? [],
    ),
  createReview: () =>
    mockOctokit("post", "/repos/step-security/test/pulls/101/reviews", 200, {}),
};
...
test("when a review is successfully created using pull-request-number", async () => {
  apiMocks.getUser();
  mockOctokit("get", "/repos/step-security/test/pulls/102", 200, {
    head: { sha: "24c5451bbf1fb09caa3ac8024df4788aff4d4974" },
  });
  mockOctokit("get", "/repos/step-security/test/pulls/102/reviews", 200, []);

  const createReview = mockOctokit(
    "post",
    "/repos/step-security/test/pulls/102/reviews",
    200,
    { id: 1 },
  );
...
test("when a review has been previously approved by user and but requests a re-review", async () => {
  ...
  apiMocks.getPull(200, {
    head: { sha: "24c5451bbf1fb09caa3ac8024df4788aff4d4974" },
    requested_reviewers: [{ login: "step-security" }],
  });
  apiMocks.getReviews(200, [
    {
      user: { login: "some" },
...
test("when the token doesn't have write permissions", async () => {
  ...
  apiMocks.getPull();
  apiMocks.getReviews();
  mockOctokit("post", "/repos/step-security/test/pulls/101/reviews", 403, {
...
test("when a user tries to approve their own pull request", async () => {
  ...
  apiMocks.getPull();
  apiMocks.getReviews();
  mockOctokit("post", "/repos/step-security/test/pulls/101/reviews", 422, {
...
test("when the token doesn't have write access to the repository", async () => {
  ...
  apiMocks.getPull();
  apiMocks.getReviews();
  mockOctokit("post", "/repos/step-security/test/pulls/101/reviews", 404, {
    message: "Not Found",
  });
